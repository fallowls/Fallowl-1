...
export default async function twilioRoutes(fastify: FastifyInstance) {
  ...
  // Parallel dialer - initiate parallel calls
  fastify.post('/dialer/parallel-call', {
    ...
    try {
      const user = await getUserFromAuth0(request.user);
      const { phoneNumbers, amd = true, contactData = {} } = request.body as any;
      
      if (!phoneNumbers || !Array.isArray(phoneNumbers) || phoneNumbers.length === 0) {
        return reply.code(400).send({ message: 'Phone numbers array is required' });
      }
      ...
      const webhookToken = generateWebhookToken(user.id);
      
      const callPromises = phoneNumbers.map(async (phone: string, index: number) => {
        const formattedPhone = phone.startsWith('+') ? phone : `+${phone}`;
        const lineId = `line-${index}`;
        const contactName = contactData[phone]?.name || 'Unknown';
        
        const callOptions: any = {
          to: formattedPhone,
          from: credentials.phoneNumber,
          url: `${baseUrl}/api/twilio/voice/parallel-dialer?token=${encodeURIComponent(webhookToken)}&lineId=${lineId}&name=${encodeURIComponent(contactName)}&userId=${user.id}`,
          method: 'POST',
          statusCallback: `${baseUrl}/api/twilio/status?token=${encodeURIComponent(webhookToken)}`,
          statusCallbackEvent: ['initiated', 'ringing', 'answered', 'completed'],
          statusCallbackMethod: 'POST',
        ...
        if (amd) {
          callOptions.machineDetection = 'DetectMessageEnd';
          callOptions.machineDetectionTimeout = 5;
          callOptions.machineDetectionSpeechThreshold = 2400;
          callOptions.machineDetectionSpeechEndThreshold = 1200;
          callOptions.machineDetectionSilenceTimeout = 2000;
        }
        ...
        try {
          ...
          const callRecord = await storage.createCall(user.id, {
            ...
            cost: '0.00',
            callQuality: null,
            contactId: contactData[phone]?.contactId || null,
            tags: [],
            priority: 'normal',
            sentiment: 'neutral',
            callPurpose: 'parallel_dialer',
            outcome: 'in-progress',
            transcript: null,
            summary: null,
            actionItems: [],
          ...
          console.error(`Failed to initiate call to ${formattedPhone}:`, error);
          return {
            success: false,
            lineId,
            phone: formattedPhone,
  ...
  // Parallel dialer - call connected
  fastify.post('/dialer/call-connected', {
    ...
    try {
      const user = await getUserFromAuth0(request.user);
      const { callSid, lineId } = request.body as any;
      
      console.log(`✅ Call connected on line ${lineId}: ${callSid}`);
      
      wsService.broadcastToUser(user.id, 'parallel_dialer_call_connected', {
        callSid,
        lineId,
        timestamp: new Date().toISOString()
      });
  ...
  fastify.post('/dialer/call-rejected', {
    ...
    try {
      ...
      console.log(`❌ Call rejected on line ${lineId}: ${callSid}`);
      
      // Cancel the call in Twilio
      const { client } = await userTwilioCache.getTwilioClient(user.id);
      await client.calls(callSid).update({ status: 'canceled' });
      
      wsService.broadcastToUser(user.id, 'parallel_dialer_call_rejected', {
        callSid,
        lineId,
        timestamp: new Date().toISOString()
      });
  ...
  // Start conference for parallel dialer
  fastify.post('/dialer/conference/start', {
    ...
    try {
      ...
      const conferenceName = `ParallelDialer-${user.id}-${Date.now()}`;
      
      await storage.setSetting(`parallel_dialer_conference_${user.id}`, {
        conferenceName,
        status: 'active',
        startTime: Date.now(),
      ...
      console.log(`🎙️ Conference started: ${conferenceName}`);
  ...
  // End conference for parallel dialer
  fastify.post('/dialer/conference/end', {
    ...
    try {
      ...
      await storage.setSetting(`parallel_dialer_conference_${user.id}`, null);
      await storage.setSetting(`parallel_dialer_primary_call_${user.id}`, null);
      
      for (let i = 0; i < 10; i++) {
        await storage.setSetting(`parallel_dialer_secondary_call_${user.id}_line-${i}`, null);
      }
      
      console.log(`🎙️ Conference ended for user ${user.id}`);
  ...
  // Check if agent has joined conference
  fastify.get('/dialer/conference/check-join', {
    config: {
      rateLimit: rateLimitConfigs.api
    },
    preHandler: async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const conferenceSetting = await storage.getSetting(`parallel_dialer_conference_${user.id}`);
      ...
      return reply.send({
        joined: conferenceData.conferenceStarted || false,
        conferenceActive: true,
        conferenceName: conferenceData.conferenceName
      });
    } catch (error: any) {
      console.error('Conference check join error:', error);
      return reply.code(500).send({ message: error.message });
    }
  });
